...
@Component({
  ...
  styles: [`
    ...
    .navbar-brand:focus-visible { outline: 2px solid #3b82f6; outline-offset: 2px; border-radius: 0.25rem; }
    ...
    .nav-link-login:hover { background: rgba(255,255,255,0.1); color: #fff; border-color: rgba(255,255,255,0.5); }
    .nav-link-login:focus-visible { outline: 2px solid #3b82f6; outline-offset: 2px; }

    main { min-height: calc(100vh - 64px); }

    /* RTL support */
    :host-context([dir="rtl"]) .navbar-inner { flex-direction: row-reverse; }
    :host-context([dir="rtl"]) .navbar-links { flex-direction: row-reverse; }
    :host-context([dir="rtl"]) .navbar-actions { flex-direction: row-reverse; }
    ...
    @media (prefers-reduced-motion: reduce) {
      .nav-link, .nav-link-login, .skip-link { transition: none; }
    }
  `]
})
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    // I18nService initializes language on construction
  }
}
